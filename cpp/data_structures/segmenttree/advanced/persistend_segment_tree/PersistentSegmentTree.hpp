...
class PersistentSegmentTree {
 protected:
  ...
  int range_query_subroutine(std::shared_ptr<Node<T>> &node, int left_index,
                             int right_index, int query_left, int query_right) {
    // outside the range
    ...
    int middle_point = (left_index + right_index) / 2;
    int left_segment =
        range_query_subroutine(node->left, left_index, middle_point, query_left,
 ...
 public:
  ...
  int range_query(int start_index, int end_index) {
    return range_query_subroutine(root, 0, origin.size(), start_index,
                                  end_index);
  }
