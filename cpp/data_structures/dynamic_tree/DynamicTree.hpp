...
class DTNode {
 private:
  ...
  // node name => each node must have a name for dealing with it => "search ,
  // sort , ..."
  std::string name;
  ...
  bool sub_full_search(std::string const& target_name) {
    if (this->name == target_name)
      ...
      for (std::shared_ptr<DTNode<T>> node : this->children) {
 ...
 public:
  ...
  std::vector<std::shared_ptr<DTNode<T>>> children;
  ...
  DTNode(std::string node_name, T node_value,
         std::shared_ptr<DTNode<T>> parent_node = NULL)
      : name{node_name}, value{node_value}, parent{parent_node} {}

  std::string get_name() { return this->name; }

  std::string get_parent_name() { return this->parent->get_name(); }

  // get length of children in this node
  std::size_t length() { return this->children.size(); }

  // remove all children of "current node"
  void remove_children() {
    // nested recursive remove_children => remove all children of children
    for (std::shared_ptr<DTNode<T>>& child : this->children) {
      child->remove_children();
    }

    // last step make a new empty vector
    this->children = std::vector<std::shared_ptr<DTNode<T>>>();
  }

  // o(1)
  // get a copy of this node without parent or children
  DTNode<T> copy() {
    DTNode<T> node_copy = *this;
    node_copy.parent = std::make_shared<DTNode<T>>();
...
class DynamicTree {
 private:
  ...
  // o(1)
  // function take a "temp node" & make a simple check if that temp include a
  // parent with specific name return => "parent pointer" or NULL
  std::shared_ptr<DTNode<T>> search_up(std::string& parent_name,
                                       std::shared_ptr<DTNode<T>> temp) {
  ...
  // o(1) --> o(log n)
  // like "search_up" function , but this one for looking "down" between
  // children
  std::shared_ptr<DTNode<T>> search_down(std::string& child_name,
                                         std::shared_ptr<DTNode<T>> temp) {
    // o(log n)
    // search for target
  ...
  */
  bool travel(std::vector<std::string> const& path_of_names, bool up = false) {
    ...
    // if "path_of_names" is valid we make it the new current_position "as last
    // step" else nothing will be happen
    std::shared_ptr<DTNode<T>> temp = current_node;
 ...
 public:
  ...
  std::shared_ptr<DTNode<T>> current_node;

  // constructor
  DynamicTree(std::string root_name, T root_value) {
    // parent of root must be null
    root = std::make_shared<DTNode<T>>(root_name, root_value);
    current_node = root;
  }

  // o( (n log n) + (log n) )
  // duplicate names are not allowed here in dynamic tree
  bool insert(std::string const new_node_name, T const new_node_value) {
    ...
    // then
    // insert new child
    this->current_node->children.push_back(std::make_shared<DTNode<T>>(
        new_node_name, new_node_value, current_node));
  ...
  void sort() {
    std::sort(
        ...
        // comparison function
        [&](std::shared_ptr<DTNode<T>>& a, std::shared_ptr<DTNode<T>>& b) {
  ...
  bool remove_child(std::string const& target_node_name) {
    ...
    if (index != -1) {
      ...
      this->current_node->children.erase(this->current_node->children.begin() +
                                         index);
      return true;
    }
  ...
  DTNode<T> get_child(std::string const& target_node_name) {
    int index = -1;
    ...
    if (index != -1) {
      ...
      copy_of_child.parent = std::make_shared<DTNode<T>>();
