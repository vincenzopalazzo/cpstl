...
static void CustomSingleArguments(benchmark::internal::Benchmark* b);
...
static void BM_BLOOM_FILTER_REPORT(benchmark::State& state) {
  for (auto _ : state) {
    ...
    state.ResumeTiming();
    cpstl::BloomFilter<long> bloomFilter(state.range(0), cpstl::ErrProb::LOW);
    for (auto elem : inputs) bloomFilter.insert(elem);
    std::size_t countFalsePositive = 0;
...
/**
 * Test the default value of the Bloom Filter implementation, so use the
 * size (m) doubled of the size of the array.
 */
static void BM_BLOOM_FILTER_REPORT_DEF(benchmark::State& state) {
  for (auto _ : state) {
    state.PauseTiming();
    auto inputGenerator =
        std::uniform_int_distribution<long>(0, state.range(0));
    ...
    state.ResumeTiming();
    cpstl::BloomFilter<long> bloomFilter(state.range(0));
    for (auto elem : inputs) bloomFilter.insert(elem);
    std::size_t countFalsePositive = 0;
    for (auto elem : notInside) {
      auto result = bloomFilter.contains(elem);
...
BENCHMARK(BM_BLOOM_FILTER_REPORT)->Apply(CustomSingleArguments)->Complexity();
BENCHMARK(BM_BLOOM_FILTER_REPORT_DEF)
    ->Apply(CustomSingleArguments)
    ->Complexity();
BENCHMARK_MAIN();
