...
class PostingPair {
 ...
 public:
  ...
  PostingPair(std::string const &name_file,
              std::shared_ptr<UniversalHash<int>> hash_function)
...
class InvertedIndex {
 private:
  ...
  void add_word(std::string const &name_file, std::string const &word) {
    ...
    posting_list.insert(std::make_pair(
        word, std::vector<internal::PostingPair>{
                  internal::PostingPair(name_file, hash_function)}));
  }
 ...
 public:
  ...
  bool add_file(std::string const &file_path) {
    ...
    if (files_name.find(file_path) != files_name.end()) return false;
  ...
  std::string to_string() {
    ...
    for (std::size_t index = 0; index < posting_list.size(); index++) {
      ...
      for (auto &file_record : posting_iter->second) {
        row += "| " + file_record.posting_item.name_file + ": " +
               std::to_string(file_record.posting_item.count) + "\n";
      }
      table += row;
