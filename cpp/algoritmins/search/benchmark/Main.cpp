...
static void BM_BS(benchmark::State& state)
{
    for(auto _ : state) {
        ...
        state.ResumeTiming();
        int index = cpstl::Search<int, int>::binary_search(random_array, random_array.at(random_array.size() - 8));
        assert(index != -1);
    }
}
...
static void BM_BS_CLOSE_TO_START(benchmark::State& state)
{
    for(auto _ : state) {
        ...
        for (int i = 0; i < state.range(0); i++)
            random_array.push_back(i);
        state.ResumeTiming();
        int index = cpstl::Search<int, int>::binary_search(random_array, random_array.at(12));
        assert(index != -1);
    }
}
...
static void BM_EX(benchmark::State& state)
{
    for(auto _ : state) {
        ...
        for (int i = 0; i < state.range(0); i++)
            random_array.push_back(i);
        state.ResumeTiming();
        int index = cpstl::Search<int, int>::exponential_search(random_array, random_array.at(random_array.size() - 8));
        //assert(index != -1);
    }
}
...
static void BM_EX_CLOSE_TO_START(benchmark::State& state)
{
    for(auto _ : state) {
        ...
        for (int i = 0; i < state.range(0); i++)
            random_array.push_back(i);
        state.ResumeTiming();
        int index = cpstl::Search<int, int>::exponential_search(random_array, random_array.at(random_array.size() - 2));
        //assert(index != -1);
    }
}
