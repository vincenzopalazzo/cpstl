...
namespace cpstl {
  ...
  private:
    ...
    void rebuild() {
      hash_function_one.new_hash_function(3 * this->size);
      hash_function_twoo.new_hash_function(3 * this->size);
      std::vector<T> values;
  ...
  public:
    CuckooHashing(std::size_t size): size(size),
                                     hash_function_one(3 * size),
                                     hash_function_twoo(3 * size)
    {}
